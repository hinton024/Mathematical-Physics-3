...
def My_Trap(f_, a, b, n):
    ...
    h = (b - a) / n
    for i in range(n + 1):
        y.append(f_(a + i * h))  # y at limit points
    trp = h * (f_(a) + f_(b)) / 2
...
def My_Simp(f_, a, b, n):
    ...
    for i in range(1, 2 * n):
        if (i % 2 == 0):
            simp = simp + 2 * h * f_(a + i * h) / 3
        elif (i % 2 == 1):
            simp = simp + 4 * h * f_(a + i * h) / 3
...
def MyTrap_tol(f_, a, b, n, d):
    ...
    err = []
    while True:
        e = abs(My_Trap(f_, a, b, i) - My_Trap(f_, a, b, i + 1))
        err.append(e)
        if e > 0.5 * 10 ** -d:
            i = i + 1
        elif e <= 0.5 * 10 ** -d:
...
def MySimp_tol(f_, a, b, n, d):
    ...
    err = []
    while True:
        e = abs(My_Simp(f_, a, b, i) - My_Simp(f_, a, b, i + 1))
        err.append(e)
        if e > 0.5 * 10 ** -d:
            i = i + 1
        elif e <= 0.5 * 10 ** -d:
...
def MyLegQuadrature(f, a, b, n, m):  # Gauss legendre Quadrature ftion
    h = (b - a) / m
    [leg_zer, w] = p_roots(n)
    leg_zer.tolist()
    w.tolist()
...
def MyLegQuadrature_tol_1(fs, a, b, n, d):
    ...
    err = []
    while True:
        e = abs(MyLegQuadrature(fs, a, b, n, i) - MyLegQuadrature(fs, a, b, n, i + 1))
        err.append(e)
        if e > 0.5 * 10 ** -d:
            i = i * 2
        elif e <= 0.5 * 10 ** -d:
